...
describe("buyer order schemas", () => {
      ...
      if (!r.success) {
        expect(
          r.error.issues.some((i) =>
            i.message.includes("1 produk atau 1 layanan"),
          ),
        ).toBe(true);
      }
    });
  ...
  describe("orderStatusSchema", () => {
    ...
    it("rejects unknown status", () => {
      expect(orderStatusSchema.safeParse({ status: "BOGUS" }).success).toBe(
        false,
      );
    });
  });
  ...
  describe("cancelOrderSchema", () => {
    it("requires reason >= 5 chars", () => {
      ...
      expect(
        cancelOrderSchema.safeParse({ reason: "berubah pikiran" }).success,
      ).toBe(true);
    });
  });
