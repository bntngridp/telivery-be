...
describe("cart schemas", () => {
  describe("addCartItemSchema", () => {
    ...
    it("rejects when jumlah < 1", () => {
      expect(
        addCartItemSchema.safeParse({ produkId: 1, jumlah: 0 }).success,
      ).toBe(false);
      expect(
        addCartItemSchema.safeParse({ produkId: 1, jumlah: -1 }).success,
      ).toBe(false);
    });
  ...
  describe("cartItemIdParamSchema", () => {
    ...
    it("rejects non-numeric", () => {
      expect(cartItemIdParamSchema.safeParse({ id: "abc" }).success).toBe(
        false,
      );
    });
