...
function signBuyer(id: number) {
    return jwt.sign({ id, role: 'pembeli' }, process.env.JWT_SECRET ?? 'dev', { expiresIn: '1d' });
}

function signSeller(id: number) {
    return jwt.sign({ sellerId: id, role: 'penjual' }, process.env.JWT_SECRET ?? 'dev', { expiresIn: '7d' });
}

function signAdmin() {
    return jwt.sign({ adminId: -1, role: 'admin' }, process.env.JWT_SECRET ?? 'dev', { expiresIn: '1d' });
}
...
describe('Auth endpoints', () => {
    ...
    it('POST /api/auth/seller/login valid creds returns token + updates last_login_at', async () => {
        ...
        console.log('All sellers in DB:', await prisma.penjual.findMany({ select: { email: true } }));
        const res = await request(app)
            .post(`${PREFIX}/auth/seller/login`)
        ...
        const updated = await prisma.penjual.findUnique({ where: { mitra_id: seller.mitra_id } });
    ...
    it('POST /api/auth/buyer/register with valid data returns 201', async () => {
        ...
        const created = await prisma.pembeli.findFirst({ where: { phone_number: phone } });
...
describe('Buyer Profile endpoints', () => {
    ...
    it('PATCH /api/buyer/profile with valid data updates DB', async () => {
        ...
        expect(res.status).toBe(200);
        const updated = await prisma.pembeli.findUnique({ where: { user_id: buyer.user_id } });
...
describe('Cart endpoints', () => {
    ...
    it('POST /api/buyer/cart/items adds produk', async () => {
        ...
        if (res.status !== 201) {
            ...
            console.error('  produkId:', produkId, 'sellerId:', sellerId, 'buyerId:', buyerId);
        }
        expect(res.status).toBe(201);
    });
    ...
    it('POST same produk merges jumlah', async () => {
        ...
        const item = res.body.data.items.find((i: { produk_id: number }) => i.produk_id === produkId);
