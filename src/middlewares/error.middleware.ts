...
function flattenZodErrors(err: ZodError): { errors: Record<string, string[]>; fieldList: FieldError[] } {
...
export function errorHandler(
    ...
    if (err instanceof HttpError) {
        fail(res, err.message, err.statusCode, isDevelopment ? String(err.details ?? '') : undefined);
        return;
    }

    if (err instanceof Prisma.PrismaClientKnownRequestError) {
        if (err.code === 'P2002') {
            fail(res, 'Duplicate entry — resource already exists', 409, err.meta ? String(err.meta) : undefined);
            return;
        }
        if (err.code === 'P2025') {
