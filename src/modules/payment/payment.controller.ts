...
export const paymentController = {
    uploadReceipt: asyncHandler(async (req: Request, res: Response) => {
        ...
        if (!req.file) throw new BadRequestError('File bukti bayar wajib diupload (field: receipt)');

        const filePath = `./documents/payment_receipts/${req.file.filename}`;
        const updated = await paymentService.uploadReceipt(Number(pesananId), userId, filePath);
    ...
    confirmPayment: asyncHandler(async (req: Request, res: Response) => {
        ...
        const updated = await paymentService.confirmPayment(Number(pembayaranId), sellerId);
    ...
    rejectPayment: asyncHandler(async (req: Request, res: Response) => {
        ...
        const updated = await paymentService.rejectPayment(Number(pembayaranId), sellerId, catatan);
