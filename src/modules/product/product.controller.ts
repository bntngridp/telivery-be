...
function requireSeller(req: Request): number {
  if (
    ...
    typeof req.user.sellerId !== "number"
  ) {
...
export const createProduct = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!kategoriRecord)
      throw new BadRequestError("Kategori tidak ditemukan di database");
...
export const getProductsByCategory = asyncHandler(
  async (req: Request, res: Response) => {
    const sellerId = requireSeller(req);
    const data = await productService.listBySellerAndCategory(
      sellerId,
      resolveCategoryDb(req.params.category),
    );
    return success(res, "Produk berhasil diambil", data);
  },
);
