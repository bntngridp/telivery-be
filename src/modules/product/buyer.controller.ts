...
export const buyerProductController = {
  getAllProducts: asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await buyerProductService.getAllProducts(
      pagination.page,
      pagination.limit,
    );
    return paginated(
  ...
  getPopularProducts: asyncHandler(async (req: Request, res: Response) => {
    const pagination = paginationSchema.parse(req.query);
    const products = await buyerProductService.getPopularProducts(
      pagination.limit,
    );
    return success(res, "Produk populer berhasil diambil", products);
  }),

  getProductRecommendations: asyncHandler(
