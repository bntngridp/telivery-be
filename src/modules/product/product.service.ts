...
export const productService = {
    ...
    async findByIdForBuyer(productId: number) {
        return prisma.produk.findFirst({
            ...
            include: {
                kategori: true,
                penjual: {
                    select: { nama_toko: true, alamat_toko: true, status_toko: true, jenis_usaha: true },
                },
            },
        });
    },
    ...
    async listBySellerAndCategory(sellerId: number, kategoriDb: string) {
        const kategoriRecord = await prisma.kategori.findFirst({ where: { nama_kategori: kategoriDb } });
    ...
    async listForBuyer() {
        return prisma.produk.findMany({
            ...
            include: {
                kategori: true,
                penjual: { select: { nama_toko: true, alamat_toko: true, status_toko: true } },
            },
    ...
    async listForBuyerByCategory(kategoriDb: string) {
        ...
        if (!kategoriRecord) throw new BadRequestError('Kategori tidak ditemukan');
        return prisma.produk.findMany({
            where: { id_kategori: kategoriRecord.id_kategori, status_ketersediaan: true, stok_produk: { gt: 0 } },
            include: {
                kategori: true,
                penjual: { select: { nama_toko: true, alamat_toko: true, status_toko: true } },
            },
    ...
    async listRecommendations(userId: number | undefined, limit = 10) {
        if (userId) {
            const userOrders = await prisma.pesanan.findMany({
                ...
                include: { detail_pesanan_produk: { include: { produk: { include: { kategori: true } } } } },
    ...
    async update(productId: number, sellerId: number, input: UpdateProductInput) {
        ...
        if (input.kategori) {
            const rec = await prisma.kategori.findFirst({ where: { nama_kategori: input.kategori } });
        ...
        if (input.pathGambar && existing.foto && input.pathGambar !== existing.foto) {
    ...
    async deleteAll(sellerId: number) {
        const products = await prisma.produk.findMany({ where: { mitra_id: sellerId } });
        for (const p of products) deleteFileIfExists(p.foto);
        const result = await prisma.produk.deleteMany({ where: { mitra_id: sellerId } });
