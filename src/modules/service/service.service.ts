...
export const serviceService = {
  ...
  async update(layananId: number, sellerId: number, data: UpdateLayananDto) {
    ...
    if (data.namaLayanan !== undefined)
      updateData.nama_layanan = data.namaLayanan;
    if (data.harga !== undefined)
      updateData.harga = new Prisma.Decimal(data.harga);
    if (data.jenisLayanan !== undefined)
      updateData.jenis_layanan = data.jenisLayanan;
    if (data.estimasiDurasi !== undefined)
      updateData.estimasi_durasi = data.estimasiDurasi.toString();
  ...
  async delete(layananId: number, sellerId: number) {
    ...
    if (referenced) {
      throw new ForbiddenError(
        "Layanan sudah pernah dipesan, tidak bisa dihapus",
      );
    }
