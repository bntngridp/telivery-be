...
function parseId(value: string, fieldName: string): number {
    ...
    if (!Number.isFinite(n) || n <= 0) throw new BadRequestError(`${fieldName} tidak valid`);
    return n;
}
...
export const serviceController = {
    ...
    getById: asyncHandler(async (req: Request, res: Response) => {
        ...
        const layanan = await serviceService.findOwnedById(parseId(id, 'id'), sellerId);
    ...
    update: asyncHandler(async (req: Request, res: Response) => {
        ...
        const updated = await serviceService.update(parseId(id, 'id'), sellerId, data);
