...
function buyerWhere(userId: number, tipe?: NotificationType): Prisma.notifikasiWhereInput {
    const where: Prisma.notifikasiWhereInput = { user_id: userId };
    if (tipe) where.tipe = tipe;
    return where;
}

function sellerWhere(sellerId: number, tipe?: NotificationType): Prisma.notifikasiWhereInput {
...
export const notificationService = {
    async getBuyerNotifications(buyerId: number, page: number, limit: number, tipe?: NotificationType) {
    ...
    async getSellerNotifications(sellerId: number, page: number, limit: number, tipe?: NotificationType) {
        const skip = (page - 1) * limit;
