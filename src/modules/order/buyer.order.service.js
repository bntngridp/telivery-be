...
exports.buyerOrderService = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
exports.buyerOrderService = {
    ...
    async getBuyerOrders(userId, page = 1, limit = 10) {
    ...
    async getOrdersByStatus(userId, status, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        const [orders, totalCount] = await Promise.all([
            prisma.pesanan.findMany({
                where: {
    ...
    async reviewOrder(orderId, userId, data) {
        const order = await prisma.pesanan.findFirst({
            where: {
                pesanan_id: orderId,
                user_id: userId,
