...
export const orderController = {
    getSellerOrders: asyncHandler(async (req: Request, res: Response) => {
        ...
        const result = await orderService.getSellerOrders(sellerId, pagination.page, pagination.limit);
        return paginated(
            res,
    ...
    acceptOrder: asyncHandler(async (req: Request, res: Response) => {
        const sellerId = requireSellerId(req);
        const params = orderIdSchema.parse(req.params);
        const updatedOrder = await orderService.acceptOrder(Number(params.id), sellerId);
        if (!updatedOrder) throw new NotFoundError('Pesanan tidak ditemukan atau tidak dapat diubah statusnya');
    ...
    rejectOrder: asyncHandler(async (req: Request, res: Response) => {
        ...
        const updatedOrder = await orderService.rejectOrder(Number(params.id), sellerId, body.reason);
        if (!updatedOrder) throw new NotFoundError('Pesanan tidak ditemukan atau tidak dapat diubah statusnya');
    ...
    processOrder: asyncHandler(async (req: Request, res: Response) => {
        const sellerId = requireSellerId(req);
        const params = orderIdSchema.parse(req.params);
        const updatedOrder = await orderService.processOrder(Number(params.id), sellerId);
        if (!updatedOrder) throw new NotFoundError('Pesanan tidak ditemukan atau tidak dapat diubah statusnya');
        return success(res, 'Status pesanan berhasil diubah menjadi diproses', updatedOrder);
    }),
    ...
    deliverOrder: asyncHandler(async (req: Request, res: Response) => {
        ...
        const updatedOrder = await orderService.deliverOrder(Number(params.id), sellerId, body.note);
        if (!updatedOrder) throw new NotFoundError('Pesanan tidak ditemukan atau tidak dapat diubah statusnya');
        return success(res, 'Status pesanan berhasil diubah menjadi dikirim', updatedOrder);
    }),
