...
export const partnerService = {
    ...
    async updateProfile(sellerId: number, data: UpdateStoreProfileDto) {
        ...
        if (data.namaToko && data.namaToko !== existing.nama_toko) {
            ...
            if (duplicate) throw new ConflictError('Nama toko sudah digunakan penjual lain');
        }

        const updateData: Prisma.penjualUpdateInput = {};
        if (data.namaToko !== undefined) updateData.nama_toko = data.namaToko;
        if (data.deskripsiToko !== undefined) updateData.deskripsi_toko = data.deskripsiToko;
        if (data.jamOprasional !== undefined) updateData.jam_oprasional = data.jamOprasional;
