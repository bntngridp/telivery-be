...
async function validateProdukForCart(produkId: number) {
    const produk = await prisma.produk.findFirst({
        where: { produk_id: produkId, status_ketersediaan: true, stok_produk: { gt: 0 } },
        include: { penjual: { select: { mitra_id: true, status_toko: true, status_verifikasi: true } } },
    });
    if (!produk) throw new BadRequestError('Produk tidak tersedia atau stok habis');
    if (!produk.penjual) throw new BadRequestError('Toko tidak ditemukan untuk produk ini');
    if (
        produk.penjual.status_verifikasi !== APPROVED_OPEN_FILTER.status_verifikasi ||
...
async function validateLayananForCart(layananId: number) {
    ...
    if (!layanan.penjual) throw new BadRequestError('Toko tidak ditemukan untuk layanan ini');
    if (
        layanan.penjual.status_verifikasi !== APPROVED_OPEN_FILTER.status_verifikasi ||
...
export const cartService = {
    ...
    async getCart(userId: number) {
        ...
        const produkIds = items.map((i) => i.produk_id).filter((id): id is number => id !== null);
        const layananIds = items.map((i) => i.layanan_id).filter((id): id is number => id !== null);
