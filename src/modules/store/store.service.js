...
exports.storeService = {
    ...
    async searchStores(keyword, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        const [stores, totalCount] = await Promise.all([
            prisma.penjual.findMany({
                where: {
    ...
    async getStoresByCategory(kategoriParam, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        let jenisUsaha;
    ...
    async getStoreRecommendations(userId, limit = 10) {
        ...
        if (userId) {
            ...
            const storeIds = userOrders.map(order => order.mitra_id).filter(Boolean);
    ...
    async getStoreProducts(storeId, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        const store = await prisma.penjual.findFirst({
            where: {
                mitra_id: storeId,
